...
def get_wechat_profile_dir(account_id: str) -> str:
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "data", "profiles", "wechat_mp", account_id))
...
class WeChatMpAdapter(BasePlatformAdapter):
    ...
    async def _find_exact_qr_element(self, page: Page):
        return page.locator(".login__type__container__scan__qrcode, .qrcode-wrp img, .login__type__container__scan img, img.qrcode, img[src*='qrcode']").locator("visible=true").first
    ...
    async def start_login_session(self, account_id: str) -> Dict[str, Any]:
        async with self._lock:
            ...
            login_session_id = f"wx_sess_{int(time.time())}_{account_id}"
            profile_dir = get_wechat_profile_dir(account_id)
    ...
    async def get_qrcode_image(self, account_id: str) -> Dict[str, Any]:
        ...
        if not sess or not sess.get("page"):
            return {"success": False, "status": STATUS_NOT_LOGIN, "errorMessage": "未找到活跃的微信公众号登录会话"}
    ...
    async def check_login_status(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_auth = any(k in cookie_names for k in ["slave", "data_ticket"]) or "token=" in current_url
            ...
            try:
                name_elem = page.locator(".weui-desktop-account__info .weui-desktop-account__nickname, .account_name, .user-name").first
                if await name_elem.count() > 0:
                    nickname = await name_elem.inner_text()
                avatar_elem = page.locator(".weui-desktop-account__avatar, .avatar_img img, .user-avatar img").first
            ...
            if not nickname:
                nickname = f"微信公众号_{account_id[-4:]}"
            ...
            enc_session = encrypt_session({
                ...
                "profile_dir": profile_dir,
                "auth_cookies": [c for c in cookies if c.get("name") in ["slave", "data_ticket", "bizuin"]]
            })
