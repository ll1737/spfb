...
def get_bilibili_profile_dir(account_id: str) -> str:
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "data", "profiles", "bilibili", account_id))
    os.makedirs(base_dir, exist_ok=True)
    return base_dir

class BilibiliAdapter(BasePlatformAdapter):
    platform_name = "bilibili"
    creator_url = "https://member.bilibili.com/platform/upload/text/edit"
    ...
    async def start_login_session(self, account_id: str) -> Dict[str, Any]:
        async with self._lock:
            ...
            login_session_id = f"bili_sess_{int(time.time())}_{account_id}"
            profile_dir = get_bilibili_profile_dir(account_id)
        ...
        try:
            ...
            await page.goto("https://passport.bilibili.com/login", wait_until="domcontentloaded", timeout=30000)
            await page.wait_for_timeout(2500)

            # Switch to QR code tab if needed
            qr_loc = page.locator("div[class*='login-scan'], div[class*='qrcode'], div[class*='scan-box'], .qrcode-login, canvas:visible").first
    ...
    async def get_qrcode_image(self, account_id: str) -> Dict[str, Any]:
        ...
        if not sess or not sess.get("page"):
            return {"success": False, "status": STATUS_NOT_LOGIN, "errorMessage": "未找到活跃的B站登录会话"}
    ...
    async def check_login_status(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            has_auth = any(k in cookie_names for k in ["SESSDATA", "bili_jct"])

            if not has_auth:
                scanned_indicator = page.locator("text=扫描成功, text=请在手机上确认, .qrcode-scan-success").first
            ...
            try:
                await page.goto("https://member.bilibili.com/platform/home", wait_until="domcontentloaded", timeout=20000)
            ...
            avatar_url = None
            try:
                name_elem = page.locator(".user-name, .nav-name, .uname").first
                if await name_elem.count() > 0:
                    nickname = await name_elem.inner_text()
                avatar_elem = page.locator(".header-avatar-wrap img, .nav-item-avatar, .face-img img").first
            ...
            if not nickname:
                nickname = f"B站UP主_{account_id[-4:]}"
            ...
            enc_session = encrypt_session({
                ...
                "profile_dir": profile_dir,
                "auth_cookies": [c for c in cookies if c.get("name") in ["SESSDATA", "bili_jct", "DedeUserID"]]
            })
            ...
            return {
                ...
                "account": {
                    ...
                    "nickname": nickname,
                    "name": nickname,
                    "avatarUrl": avatar_url or "https://static.hdslb.com/images/member/noface.gif",
                    "status": "active",
                    "encryptedSession": enc_session,
                    "sessionPreview": f"Persistent Profile ({profile_dir}) (已认证)",
    ...
    async def _check_profile_disk_status(self, profile_dir: str):
        ...
        try:
            ...
            has_auth = any(c.get("name") in ["SESSDATA", "bili_jct"] for c in cookies)
            return has_auth, "B站UP主" if has_auth else None
        except Exception:
            return False, None

    async def validate_session(self, account: Dict[str, Any]) -> SessionStatus:
        account_id = account.get("id", "default_bili")
        profile_dir = get_bilibili_profile_dir(account_id)
        is_valid, nickname = await self._check_profile_disk_status(profile_dir)
        return SessionStatus(
            ...
            platform=self.platform_name,
            account_id=account_id,
            nickname=nickname or account.get("nickname", "B站UP主"),
            error=None if is_valid else "未检测到有效B站登录 Profile 或 SESSDATA Cookie"
        )

    async def publish_article(self, payload: Dict[str, Any], account: Dict[str, Any]) -> PublishResult:
        account_id = account.get("id", "default_bili")
        task_id = payload.get("taskId", f"task_bili_{int(time.time())}")
        profile_dir = get_bilibili_profile_dir(account_id)

        logs = []
        logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "info", "message": f"正在加载B站专属 Persistent Profile: {profile_dir}"})
        ...
        try:
            ...
            logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "info", "message": "导航至B站专栏编辑器: " + self.creator_url})
            await page.goto(self.creator_url, wait_until="domcontentloaded", timeout=30000)
            await page.wait_for_timeout(3000)

            if "login" in page.url:
                raise Exception("B站登录态已失效，请重新扫码登录")

            # Title
            title_input = page.locator("input[placeholder*='请输入标题'], .title-input, textarea[placeholder*='标题']").first
            if await title_input.count() > 0 and await title_input.is_visible():
                title = payload.get("title", "")
                logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "info", "message": f"填充B站专栏标题: {title}"})
                await title_input.fill(title)

            # Editor Content
            editor = page.locator(".ProseMirror, .ql-editor, div[contenteditable='true']").first
            if await editor.count() > 0:
                content = payload.get("content", "") or payload.get("summary", "")
                logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "info", "message": "填充B站专栏正文..."})
                await editor.click()
            ...
            # Submit button
            logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "info", "message": "点击【提交专栏/发布】按钮..."})
            submit_btn = page.locator("button:has-text('提交专栏'), button:has-text('发布'), .submit-btn").last
            if await submit_btn.count() > 0:
                await submit_btn.click()
                await page.wait_for_timeout(4000)

            real_url = page.url
            logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "success", "message": f"B站专栏真实发布成功！当前地址: {real_url}"})
            ...
            if pw:
                try:
                    ...
                    pass
            logs.append({"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "level": "error", "message": f"B站自动化发布异常: {str(e)}"})
            return PublishResult(
                success=False,
                platform=self.platform_name,
                status="failed",
